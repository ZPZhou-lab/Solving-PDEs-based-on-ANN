...
# %% [markdown]
# ## Problem 1
# ### 方程类型：$\phi\cdot\phi_x + v\cdot \phi_y = \phi_{xx}+\phi_{yy} - (2\sin{y}+\sin{x})\cos{x}$
# ### 其中，$x,y\in[0,\pi]$, $v=\sin{x}\cos{y}$, 解析解$\phi=-\cos{x}\sin{y}$
# ### 由解析解给出下面的边界条件：
# $\phi(0,y)=-\sin{y}$, $\phi(\pi,y)=\sin{y}$, $\phi(x,0)=0$, $\phi(x,\pi)=0$
...
m = n_use
n = n_use
X_train = np.linspace(0,np.pi,m,endpoint=True) # 生成[0,1]区间m个点
Y_train = np.linspace(0,np.pi,n,endpoint=True) # 生成[0,1]区间n个点
# 生成格点(x_j,y_j)
...
# 生成边界训练点
n_bc = n_use
x0_train = np.hstack((np.zeros([n_bc,1]),np.reshape(np.linspace(0,np.pi,n_bc),[n_bc,1])))
x1_train = np.hstack((np.pi*np.ones([n_bc,1]),np.reshape(np.linspace(0,np.pi,n_bc),[n_bc,1])))
y0_train = np.hstack((np.reshape(np.linspace(0,np.pi,n_bc),[n_bc,1]),np.zeros([n_bc,1])))
y1_train = np.hstack((np.reshape(np.linspace(0,np.pi,n_bc),[n_bc,1]),np.pi*np.ones([n_bc,1])))
...
p = 51
q = 51
X_test = np.linspace(0,np.pi,p,endpoint=True) # 生成[0,1]区间p个点
Y_test = np.linspace(0,np.pi,q,endpoint=True) # 生成[0,1]区间q个点
# 生成测试格点(x_j,y_j)
...
# %%
# 计算边界训练点真值
bc_train = -np.cos(x_train_bc)*np.sin(y_train_bc)
bc_train = np.reshape(bc_train,[bc_train.shape[0],1])
# 计算测试集真值
real_test = -np.cos(x_test)*np.sin(y_test)
...
# %%
# 定义残差函数(导数增长损失函数)
def residual(Psi_xx,Psi_yy,Psi_x,Psi_y,x,y):
    loss_e = tf.reduce_mean(((-tf.cos(x)*tf.sin(y))*Psi_x+(tf.sin(x)*tf.cos(y))*Psi_y
                            -(Psi_xx + Psi_yy) + (2*tf.sin(y)+tf.sin(x))*tf.cos(x))**2)
    return loss_e


# %%
# 定义边界损失函数
def loss_bc(model,x,y,bc_train):
    loss_b = tf.reduce_mean((model(tf.concat([x,y],1)) - bc_train)**2)
    return loss_b
...
for epoch in range(EPOCHS):
    with tf.GradientTape(persistent=True) as tape1:
        ...
        loss_e = residual(Psi_xx=Psi_xx,Psi_yy=Psi_yy,Psi_x=Psi_x,Psi_y=Psi_y,x=x,y=y)
        loss_b = loss_bc(model=model,x=x_train_bc,y=y_train_bc,bc_train=bc_train)
...
# %%
# 绘制误差图样
def errorplot1(model,loss,loss_e,loss_b,alpha,rate=0.9): 
...
# %%
errorplot1(model,loss=loss_list,loss_e=loss_e_list,loss_b=loss_b_list,alpha=alpha_list,rate=0.8)


# %%
# L-BFGS生成函数
def function_factory(model, residual, loss_bc, x, y, x_train_bc, y_train_bc, bc_train, alpha):
...
L_BFGS_Optimizer = function_factory(model=model, residual=residual, loss_bc=loss_bc, x=x, y=y, 
                                    x_train_bc=x_train_bc, y_train_bc=y_train_bc, bc_train=bc_train,alpha=alpha)
